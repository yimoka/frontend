 ...
 * 此脚本执行以下操作：
 * 1. 清理旧的构建文件
 * 2. 使用EAS构建Android生产本地客户端（支持AAB和APK格式）
...
function askBuildType() {
  return new Promise((resolve) => {
    ...
    rl.question(colors.yellow('请输入选项 [1-3] (默认: 2): '), (answer) => {
      const option = answer.trim() || '2';

      if (['1', '2', '3'].includes(option)) {
        resolve(option);
      } else {
        console.log(colors.red('无效选项，使用默认选项2 (APK格式)'));
        resolve('2');
      }
    });
  });
...
async function main() {
  try {
    ...
    if (buildType === '1' || buildType === '3') {
      buildProfiles.push('prodLocal-aab'); // AAB格式
    }
    if (buildType === '2' || buildType === '3') {
      buildProfiles.push('prodLocal-apk'); // APK格式
    }
    ...
    if (fs.existsSync(easJsonPath)) {
      ...
      // 添加prodLocal配置
      if (!easJson.build['prodLocal-apk']) {
        easJson.build['prodLocal-apk'] = {
          ...easJson.build.production,
          distribution: "internal",
          android: {
            ...easJson.build.production.android,
            buildType: "apk"
          }
        };
      }

      if (!easJson.build['prodLocal-aab']) {
        easJson.build['prodLocal-aab'] = {
          ...easJson.build.production,
          distribution: "internal",
          android: {
            ...easJson.build.production.android,
    ...
    try {
      ...
      for (const profile of buildProfiles) {
        const isApk = profile === 'prodLocal-apk';
        console.log(colors.blue(`\n开始构建Android ${isApk ? 'APK' : 'AAB'} 生产本地客户端...`));
    ...
    // 输出成功信息
    console.log('\n');
    console.log(colors.green('✅ Android生产本地客户端构建成功!'));
    console.log('\n');
    console.log(colors.blue('生产本地构建文件位置:'));
    buildFiles.forEach(file => {
