...
import { each } from '@yimoka/designable-shared';
import { observable } from '@yimoka/store';
...
const DESIGNER_GlobalRegistry = {
  ...
  getDesignerBehaviors: (node: TreeNode) => DESIGNER_BEHAVIORS_STORE.value.filter(pattern => pattern.selector(node)),

  getDesignerIcon: (name: string) => DESIGNER_ICONS_STORE[name],

  getDesignerLanguage: () => getISOCode(DESIGNER_LANGUAGE_STORE.value),
