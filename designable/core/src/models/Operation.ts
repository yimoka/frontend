...
export class Operation {
  ...
  dragWith(point: IPoint, touchNode?: TreeNode) {
    const { viewport } = this.workspace;
    const { outline } = this.workspace;
  ...
  cloneNodes(nodes: TreeNode[]) {
    ...
    const filterNestedNode = this.sortNodes(nodes).filter(node => !nodes.some(parent => node.isMyParents(parent)));
