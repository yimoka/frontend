...
const resetParent = (node: TreeNode, parent: TreeNode) => resetNodesParent([node], parent)[0];
...
export class TreeNode {
  ...
  get descendants(): TreeNode[] {
    return this.children.reduce((buf, node) => buf.concat(node).concat(node.descendants), []);
  }
  ...
  clone(parent?: TreeNode) {
    ...
    newNode.children = resetNodesParent(
      this.children.map(child => child.clone(newNode)),
      newNode,
    );
    return this.triggerMutation(
      new CloneNodeEvent({
  ...
  from(node?: ITreeNode) {
    ...
    return this.triggerMutation(
      ...
      () => {
        ...
        if (node.children) {
          this.children = node.children?.map?.(node => new TreeNode(node, this)) || [];
        }
      },
    );
  }
  ...
  serialize(): ITreeNode {
    return {
      ...
      props: toJS(this.props),
      hidden: this.hidden,
      children: this.children.map(treeNode => treeNode.serialize()),
    };
  }
